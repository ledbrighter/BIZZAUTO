...
router.post('/export/csv', async (req: Request, res: Response) => {
  try {
    const { businessId, leadIds } = req.body;
 ...
 */
router.post('/bulk-reply', async (req: Request, res: Response) => {
  try {
    const { businessId, leadIds, channel, message } = req.body;
    if (!businessId || !leadIds?.length || !channel || !message) {
      return res.status(400).json({ success: false, error: 'businessId, leadIds, channel, and message are required' });
    }
